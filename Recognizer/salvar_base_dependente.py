...
for i in range(1,len(nomes)):
    if (nomes[i].find('_7.wav') > -1 or nomes[i].find('_8.wav') > -1 or 
                nomes[i].find('_9.wav') > -1 or nomes[i].find('_10.wav') > -1):
        continue
...
with open('base_fbank_d.pkl', 'wb') as f:
...
for i in range(1,len(nomes)):
    if (nomes[i].find('_7.wav') == -1 and nomes[i].find('_8.wav') == -1 and 
                nomes[i].find('_9.wav') == -1 and nomes[i].find('_10.wav') == -1):
        continue
    label = montar_label(fonemas[i],tamanho,overlap)
    word = seq_frame_word(extrair(nomes[i]),n)
    label = np.concatenate([label, np.repeat(id_fonema('sil'),math.fabs(label.shape[0]-word.shape[0]))])
...
with open('base_fbank_teste_d.pkl', 'wb') as f:
    dill.dump((frames,np.append([],labels)), f)
